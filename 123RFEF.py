...
def cargar_escudos():
    try:
        ...
        # URL de descarga directa del archivo en GitHub
        github_url_escudos = "https://github.com/RobertoRicobaldi/123RFEF/raw/main/00_Modelo%20de%20datos%20-%20Primera%20RFEF%2024-25.xlsx"
...
if pagina == "Filtros y Datos":
   ...
   # Agregar una sección de "Registro Manual de Jugadora"
def registrar_jugadora():
    st.write("### Registro Manual de Jugadora")
    
    # Inicializar la columna VALORACION si no existe
    if "VALORACION" not in df.columns:
        df["VALORACION"] = None
    
    # Seleccionar una jugadora con un key único para evitar el conflicto
    jugadora_seleccionada = st.selectbox("Selecciona una jugadora", df["NOMBRE"].unique(), key="jugadora_seleccionada")
    
    if jugadora_seleccionada:
        # Filtrar los datos de la jugadora seleccionada
        jugadora_data = df[df["NOMBRE"] == jugadora_seleccionada]
        
        # Mostrar los datos de la jugadora seleccionada
        st.write(f"#### Datos de {jugadora_seleccionada}")
        st.write(jugadora_data)
        
        # Registrar puntos fuertes con un key único
        puntos_fuertes = st.text_area("Puntos Fuertes", key=f"puntos_fuertes_{jugadora_seleccionada}")
        
        # Registrar puntos de mejora con un key único
        puntos_mejora = st.text_area("Puntos Mejora", key=f"puntos_mejora_{jugadora_seleccionada}")
        
        # Registrar valoración (solo opciones 3, 5, 7, 9) con un key único
        valoracion = st.selectbox("Valoración (Elige un valor)", [3, 5, 7, 9], key=f"valoracion_{jugadora_seleccionada}")
        
        # Botón para guardar los datos con un key único
        if st.button("Guardar Registro", key=f"guardar_registro_{jugadora_seleccionada}"):
            # Guardar la valoración y los puntos de mejora
            df.loc[df["NOMBRE"] == jugadora_seleccionada, "VALORACION"] = valoracion
            st.success(f"Registro guardado para {jugadora_seleccionada}.")
            # Mostrar el resumen del registro
            st.write("### Resumen del Registro:")
            st.write(f"**Puntos Fuertes:** {puntos_fuertes}")
            st.write(f"**Puntos Mejora:** {puntos_mejora}")
            st.write(f"**Valoración:** {valoracion}")
    
    # Mostrar todas las jugadoras con sus valoraciones
    st.write("### Jugadoras y sus valoraciones")
    st.write(df[["NOMBRE", "VALORACION"]])

# Llamar a la función de registro manual de jugadora
registrar_jugadora()
